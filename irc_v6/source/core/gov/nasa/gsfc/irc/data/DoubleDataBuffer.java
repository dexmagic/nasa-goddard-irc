...
/**
 * A DoubleDataBuffer is a linear, finite sequence of <code>double</code> 
 * primitive elements.  
 *
 ...
 * for the Instrument Remote Control (IRC) project.
 *
 * @version	$Date: 2006/03/31 16:21:26 $
 * @author 	Troy Ames
 */
final class DoubleDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private double [] fNativeArray = null;

	/**
	 *	Constructs a DoubleDataBuffer having the given array of data and as 
	 ...
	 */
	public DoubleDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new double[capacity], 0, capacity);
	}

	/**
	 * Constructs a DoubleDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		double[] arrayCopy = new double[sourceCapacity];
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Number))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = doubleValue;
				}
			}
			else
			{
	...
	{
		return (new Double(fNativeArray[resolveIndex(index)]));
	}
	...
	public ByteBuffer getAsByteBuffer()
	{
		...
		for (int i = 0; i < size; i++)
		{
			buffer.putDouble(getAsDouble(i));
		}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Double[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Double(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (double) value;
     ...
     * <code>{@link Double#parseDouble(java.lang.String) Double.parseDouble(String)}</code>
	...
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Number) value).doubleValue();
		}
		...
		else
		{
			throw new IllegalArgumentException(
				"value parameter must be of type Number for this DataBuffer: " + ((value != null) ? value.getClass().getName() : "null"));
		}
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsDouble(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				double [] sampledArray = new double[numSubsamples];
				int element = 0;
...
//--- Development History  ---------------------------------------------------
//
//  $Log: DoubleDataBuffer.java,v $
//  Revision 1.10  2006/03/31 16:21:26  smaher_cvs
//  Added detail to a thrown exception.
//
//  Revision 1.9  2006/02/08 20:36:28  chostetter_cvs
//  Added initialize(Object value) method
//
//  Revision 1.8  2006/02/07 18:03:20  chostetter_cvs
//  Clears to 0
//
