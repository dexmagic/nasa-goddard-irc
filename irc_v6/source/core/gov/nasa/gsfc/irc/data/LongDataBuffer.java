...
/**
 * A LongDataBuffer is a linear, finite sequence of <code>long</code> 
 * primitive elements.  
 *
 ...
 * @author 	Troy Ames
 */
final class LongDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private long [] fNativeArray = null;

	/**
	 *	Constructs a LongDataBuffer having the given array of data and as 
	 ...
	 */
	public LongDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new long[capacity], 0, capacity);
	}

	/**
	 * Constructs a LongDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		long[] arrayCopy = new long[sourceCapacity];
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Number))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = longValue;
				}
			}
			else
			{
	...
	{
		return (new Long(fNativeArray[resolveIndex(index)]));
	}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Long[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Long(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (long) value;
     ...
     * <code>{@link Long#decode(java.lang.String) Long.decode(String)}</code>
     ...
     *          If the value cannot be converted into an <code>long</code>.
     * 
     * @see Number
     * @see Long
     */
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Number) value).longValue();
		}
		else if (value instanceof String)
		{
			try
			{
				fNativeArray[resolveIndex(index)] = 
					Long.decode((String) value).longValue();
			}
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsLong(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				long [] sampledArray = new long[numSubsamples];
				int element = 0;
