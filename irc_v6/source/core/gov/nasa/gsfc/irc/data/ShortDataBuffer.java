...
/**
 * A ShortDataBuffer is a linear, finite sequence of <code>short</code> 
 * primitive elements.  
 *
 ...
 * @author 	Troy Ames
 */
final class ShortDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private short [] fNativeArray = null;

	/**
	 *	Constructs a ShortDataBuffer having the given array of data and as 
	 ...
	 */
	public ShortDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new short[capacity], 0, capacity);
	}

	/**
	 * Constructs a ShortDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		short[] arrayCopy = new short[sourceCapacity];
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Number))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = shortValue;
				}
			}
			else
			{
	...
	{
		return (new Short(fNativeArray[resolveIndex(index)]));
	}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Short[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Short(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (short) value;
     ...
     * <code>{@link Short#decode(java.lang.String) Short.decode(String)}</code>
	...
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Number) value).shortValue();
		}
		else if (value instanceof String)
		{
			try
			{
				fNativeArray[resolveIndex(index)] = 
					Short.decode((String) value).shortValue();
			}
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsShort(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				short [] sampledArray = new short[numSubsamples];
				int element = 0;
