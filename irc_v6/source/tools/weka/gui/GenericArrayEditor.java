 ...
 * property editors.
 * <p>
 * I added support for primitive types (which had problems when
 * all elements were deleted) and call to SwingEditorSupport.init()
 * which was needed by SwingIntegerEditor to work.  smaher
 * 
...
implements PropertyEditor, ListDataListener, ListSelectionListener {
    ...
    private JButton m_ReplaceBut = new JButton("Replace");
    
    private JButton m_UpBut = new JButton("Up");
    private JButton m_DownBut = new JButton("Down");
    
    /** Should row numbers (zero-based) be shown next to array entries? */
    private static final boolean SHOW_ROW_NUMBER = true;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (e.getSource() == m_DeleteBut) {
                ...
                if (selected != null) {
                    ...
                    Object copy[] = new Object[m_ListModel.getSize()];
                    ...
                    for (int j = 0; j < copy.length; j++)
                    {
                        ...
                        if (shouldBeAdded)
                        {
                            m_ListModel.addElement(copy[j]);
                        }
                    }
                    // SFM
                    // Don't fire event or PropertyColumnModel will call stopCellEditing()
//                    m_Support.firePropertyChange("", null, null);
                }

            } else if (e.getSource() == m_ReplaceBut) {
                int [] selected = m_ElementList.getSelectedIndices();
                if (selected != null) {
                    ...
                    for (int i = 0; i < selected.length; i++) {
                        ...
                        try {
                            ...
                            Object newCopy = so.getObject();
                            m_ListModel.set(current, newCopy);
                ...
                m_ListModel.insertElementAt(m_ListModel.remove(selected), (selected > 0) ? selected - 1 : 0);
                m_ElementList.setSelectedIndex((selected > 0) ? selected - 1 : 0);
            
            } else if (e.getSource() == m_DownBut) {
                int selected = m_ElementList.getSelectedIndex();
                int destinationIndex = (selected == m_ListModel.getSize() - 1) ? selected : selected + 1;
                m_ListModel.insertElementAt(m_ListModel.remove(selected), destinationIndex);
                m_ElementList.setSelectedIndex(destinationIndex);
...
//                    m_Support.firePropertyChange("", null, null);
    ...
    /** Keep reference around to update RowHeader */
    ...
    public GenericArrayEditor() {
        ...
        m_ReplaceBut.addActionListener(m_InnerActionListener);
        ...
        m_ReplaceBut.setToolTipText("Replace the selected item on the list");
        m_DeleteBut.setToolTipText("Delete the selected list item");
        m_UpBut.setToolTipText("Move selected item up one position");
        m_DownBut.setToolTipText("Move selected item down one position");
    }
    ...
    private void updateEditorType(Object o) {
        ...
        if ((o != null) && (o.getClass().isArray())) {
            ...
            if (editor != null) {
                ...
                {
                    ((SwingEditorSupport)editor).init(null);
                }
                ...
                if(Array.getLength(o) > 0) {
                    ...
                    if (editor instanceof GenericObjectEditor) {
                        ...
                        try {
                            ...
                            {
                                newInstance = new Long(0);
                            }
                            else if (elementClass == float.class)
                            {
                                newInstance = new Float(0);
                            }
                            else if (elementClass == double.class)
                            {
                                newInstance = new Double(0);
                            }
                            else 
                            {
                                newInstance = elementClass.newInstance();
                            }
                            editor.setValue(newInstance);
                        } catch(Exception ex) {
            ...
            if (view == null) {
                ...
                btnPanel.add(m_AddBut);
                btnPanel.add(m_ReplaceBut);
//                panel.add(m_AddBut, BorderLayout.EAST);
                panel.add(btnPanel, BorderLayout.EAST);
                add(panel, BorderLayout.NORTH);
                
                
                fScrollPane = new JScrollPane(m_ElementList);
                updateRowHeader();
                ...
                btnPanel = new JPanel();
                btnPanel.add(m_DeleteBut);
                btnPanel.add(m_UpBut);
                btnPanel.add(m_DownBut);
//                add(m_DeleteBut, BorderLayout.SOUTH);
                ...
                m_ListModel.addListDataListener(this);
                m_ElementList.addListSelectionListener(this);
    ...
    public static void main(String [] args) {
        ...
        try {
            ...
            //      final String[] initial = new String[] {"Maryann", "Maher"};
            final int[] initial = new int[] {1,2,3};
            /*
             {
    ...
    /**
     * Create scroll pane row header to show index number in array
     */
    private void updateRowHeader()
    {
        ...
        {            
            ...
            String rowCount[] = new String[m_ListModel.getSize()];
            for (int i = 0; i < rowCount.length; i++)
            {
                rowCount[i] = new String("" + i + " :   ");
            }

            fRowHeader = new JList(rowCount);
            fRowHeader.setBackground(new Color(230,230,230));               
            fScrollPane.setRowHeaderView(fRowHeader);
            fScrollPane.getRowHeader().updateUI();
//            fScrollPane.validate();
//            fScrollPane.doLayout();
        }
    }
    ...
    public void valueChanged(ListSelectionEvent e)
    {
        ...
        if (m_ElementList.getSelectedIndex() != -1 )
        {
            ...
            if (m_ElementList.getSelectedIndices().length == 1) 
            {
                if (m_ElementList.getSelectedIndex() != 0)
                {
                    m_UpBut.setEnabled(true);
                }
                
                if (m_ElementList.getSelectedIndex() != m_ListModel.getSize() - 1)
                {
