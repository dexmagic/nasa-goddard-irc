...
/**
 *  A Scale multiplies each value in an array by a scale factor, 
 *  and copies the resulting value to the destination array.
 *  
...
public class Scale implements Calculation
{

	final double fFactor;

	/**
	 * Create a new ScaleCalculation. 
	 * <P>
	...
	**/
	...
	{
		//---Decrement to support (possibly) slightly faster pre-increment
		dstOff--;
		srcOff--;
		...
		while (count >= 4)
		{
			dst[++dstOff] = (double)(fFactor * src[++srcOff]);
			dst[++dstOff] = (double)(fFactor * src[++srcOff]);
			dst[++dstOff] = (double)(fFactor * src[++srcOff]);
			dst[++dstOff] = (double)(fFactor * src[++srcOff]);
			count -=4;
	 ...
	 * @param   count		Number of elements to process.
	**/
	public void apply(float[] src, int srcOff, float[] dst, int dstOff, int count)
	{
		//---Decrement to support (possibly) slightly faster pre-increment
		dstOff--;
		srcOff--;
		...
		while (count >= 4)
		{
			dst[++dstOff] = (float)(fFactor * src[++srcOff]);
			dst[++dstOff] = (float)(fFactor * src[++srcOff]);
			dst[++dstOff] = (float)(fFactor * src[++srcOff]);
			dst[++dstOff] = (float)(fFactor * src[++srcOff]);
			count -=4;
