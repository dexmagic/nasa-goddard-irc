...
/**
 * A ByteDataBuffer is a linear, finite sequence of <code>byte</code> 
 * primitive elements.  
 *
 ...
 * @author 	Troy Ames
 */
final class ByteDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private byte [] fNativeArray = null;

	/**
	 *	Constructs a ByteDataBuffer having the given array of data and as 
	 ...
	 */
	public ByteDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new byte[capacity], 0, capacity);
	}

	/**
	 * Constructs a ByteDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		byte[] arrayCopy = new byte[sourceCapacity];
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Number))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = byteValue;
				}
			}
			else
			{
	...
	{
		return (new Byte(fNativeArray[resolveIndex(index)]));
	}
	...
	public ByteBuffer getAsByteBuffer()
	{
		...
		ByteBuffer buffer = ByteBuffer.wrap(arrayCopy);
	...
	public int[] getAsIntArray()
	{
		...
		for (int i = 0; i < size; i++)
		{
			arrayCopy[i] = (int) fNativeArray[resolveIndex(i)];
		}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Byte[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Byte(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (byte) value;
    ...
    /* (non-Javadoc)
     * @see gov.nasa.gsfc.irc.data.DataBuffer#put(byte, int)
     */
     ...
     * <code>{@link Byte#decode(java.lang.String) Byte.decode(String)}</code>
     ...
     *          If the value cannot be converted into an <code>byte</code>.
     * 
     * @see Number
     * @see Integer
     */
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Number) value).byteValue();
		}
		else if (value instanceof String)
		{
			try
			{
				fNativeArray[resolveIndex(index)] = 
					Byte.decode((String) value).byteValue();
			}
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsByte(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				byte [] sampledArray = new byte[numSubsamples];
				int element = 0;
...
//
//  Revision 1.2  2005/07/15 21:25:41  tames
