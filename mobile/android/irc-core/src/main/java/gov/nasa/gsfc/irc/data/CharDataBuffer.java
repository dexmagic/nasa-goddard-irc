...
/**
 * A CharDataBuffer is a linear, finite sequence of <code>char</code> 
 * primitive elements.  
 *
 ...
 * @author 	Troy Ames
 */
final class CharDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private char [] fNativeArray = null;

	/**
	 *	Constructs a CharDataBuffer having the given array of data and as 
	 ...
	 */
	public CharDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new char[capacity], 0, capacity);
	}

	/**
	 * Constructs a CharDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		char[] arrayCopy = new char[sourceCapacity];
	 ...
	 * be a Character. If this DataBuffer is read-only, this call has no effect, and 
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Character))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = charValue;
				}
			}
			else
			{
				String message = "Value " + value + " is not a Character";
	...
	{
		return (new Character(fNativeArray[resolveIndex(index)]));
	}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Character[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Character(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (char) value;
     ...
     * <code>String</code> and <code>Character</code>. The 
     * <code>{@link String#charAt(int) String.charAt(0)}</code>
     * method is called for values of type <code>String</code> to convert to a
     * <code>char</code>.
     * 
     ...
     *          If the value cannot be converted into a <code>char</code>.
     * 
     * @see Character
     * @see String
     */
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Character) value).charValue();
		}
		else if (value instanceof String)
		{
			fNativeArray[resolveIndex(index)] = ((String) value).charAt(0);
		}
		else
		{
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsChar(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				char [] sampledArray = new char[numSubsamples];
				int element = 0;
