...
/**
 * A FloatDataBuffer is a linear, finite sequence of <code>float</code> 
 * primitive elements.  
 *
 ...
 * @author 	Troy Ames
 */
final class FloatDataBuffer extends AbstractDataBuffer
{
	/**
	...
	private float [] fNativeArray = null;

	/**
	 *	Constructs a FloatDataBuffer having the given array of data and as 
	 ...
	 */
	public FloatDataBuffer(DataBufferDescriptor descriptor, int capacity)
	{
		this(descriptor, new float[capacity], 0, capacity);
	}

	/**
	 * Constructs a FloatDataBuffer having the given array of data with the 
	...
	public DataBuffer copy()
	{
		...
		float[] arrayCopy = new float[sourceCapacity];
		...
		throws UnsupportedOperationException, IllegalArgumentException
	{
		if (! isReadOnly())
		{
			if ((value != null) && (value instanceof Number))
			{
				...
				for (int i = 0; i < size; i++)
				{
					fNativeArray[resolveIndex(i)] = floatValue;
				}
			}
			else
			{
	...
	{
		return (new Float(fNativeArray[resolveIndex(index)]));
	}
	...
	{
		int size = getSize();
		Object [] arrayCopy = new Float[size];		
		
		for (int i= 0; i < size; i++)
		{
			arrayCopy[i] = new Float(fNativeArray[resolveIndex(i)]);
		}
	...
	public DataBuffer put(int index, byte value)
	{
		...
		fNativeArray[resolveIndex(index)] = (float) value;
    	
    	return this;
	}

	/* (non-Javadoc)
	 * @see gov.nasa.gsfc.irc.data.DataBuffer#put(float, char)
	 */
     ...
     * <code>{@link Float#parseFloat(java.lang.String) Float.parseFloat(String)}</code>
     ...
     *          If the value cannot be converted into an <code>float</code>.
     * 
     * @see Number
     * @see Float
     */
	public DataBuffer put(int index, Object value)
	{
		...
		{
			fNativeArray[resolveIndex(index)] = ((Number) value).floatValue();
		}
    ...
    public DataBuffer put(int index, DataBuffer source, int sourceIndex, int length)
	{
		...
		{
			put(index + i, source.getAsFloat(sourceIndex + i));
		}
	...
	public DataBuffer downsample(int downsamplingRate)
	{
		...
		if (downsamplingRate > 1)
		{
			...
			if (downsamplingRate <= dataBufferSize)
			{
				...
				float [] sampledArray = new float[numSubsamples];
				int element = 0;
